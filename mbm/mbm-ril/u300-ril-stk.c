...
int init_stk_service(void)
{
    ...
    if (err != AT_NOERROR) {
        RLOGE("%s() Failed to activate (U)SAT profile", __func__);
        rilresponse = RIL_E_GENERIC_FAILURE;
    }
...
static char *buildStkMenu(struct stkmenu *cmenu, int n)
{
    ...
    if (!resp) {
        RLOGD("%s() Memory allocation error", __func__);
        return NULL;
    }
...
void getCachedStkMenu(void)
{
    ...
    RLOGD("%s() STKMENU: %s", __func__, resp);
...
static void sendRefreshTerminalResponse(void *param)
{
    ...
    if (!refreshState)
        RLOGD("%s() called with null parameter", __func__);
    ...
    if (err != AT_NOERROR)
        RLOGD("%s() Failed sending at command", __func__);
}
...
static void sendSimRefresh(struct tlv *tlvRefreshCmd, char *end)
{
    ...
    case SAT_NAA_SESSION_RESET:
        ...
        if ((err >= 0) && (tlvFileList.tag == 0x12)) {
            RLOGD("%s() found File List tag", __func__);
    ...
    default:
        RLOGD("%s() fallback to SIM initialization", __func__);
...
static int getCmd(char *s, struct tlv *tlvBer, struct tlv *tlvSimple)
{
    ...
    if (err < 0) {
        RLOGD("%s() error parsing BER tlv", __func__);
        return cmd;
    }

    if (tlvBer->tag == 0xD0) {
        RLOGD("%s() Found Proactive SIM command tag", __func__);
        err = mbm_parseTlv(tlvBer->data, tlvBer->end, tlvSimple);
        if (err < 0) {
            RLOGD("%s() error parsing simple tlv", __func__);
            return cmd;
        }

        if (tlvSimple->tag == 0x81) {
            RLOGD("%s() Found command details tag", __func__);
...
static int getStkResponse(char *s, struct tlv *tlvBer, struct tlv *tlvSimple)
{
    ...
    switch (cmd){
        case 0x13:
            RLOGD("%s() Send short message", __func__);
            return RIL_UNSOL_STK_EVENT_NOTIFY;
            break;
        case 0x11:
            RLOGD("%s() Send SS", __func__);
            return RIL_UNSOL_STK_EVENT_NOTIFY;
            break;
        case 0x12:
            RLOGD("%s() Send USSD", __func__);
            return RIL_UNSOL_STK_EVENT_NOTIFY;
            break;
        case 0x40:
            RLOGD("%s() Open channel", __func__);
            return RIL_UNSOL_STK_EVENT_NOTIFY;
            break;
        case 0x41:
            RLOGD("%s() Close channel", __func__);
            return RIL_UNSOL_STK_EVENT_NOTIFY;
            break;
        case 0x42:
            RLOGD("%s() Receive data", __func__);
            return RIL_UNSOL_STK_EVENT_NOTIFY;
            break;
        case 0x43:
            RLOGD("%s() Send data", __func__);
            return RIL_UNSOL_STK_EVENT_NOTIFY;
            break;
        case 0x44:
            RLOGD("%s() Get channel status", __func__);
            return RIL_UNSOL_STK_EVENT_NOTIFY;
            break;
        default:
            RLOGD("%s() Proactive command", __func__);
            break;
    }
...
error:
    RLOGE("%s() failed to parse proactive command!", __func__);
    free(line);
}
...
error:
    RLOGW("%s() Failed to parse STK Notify Event", __func__);
    free(line);
}
