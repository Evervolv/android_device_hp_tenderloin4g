...
static int getNumRetries (int request) {
    ...
    if (err != AT_NOERROR) {
        RLOGE("%s() AT*EPIN error", __func__);
        return -1;
    }
...
static UICC_Type getUICCType(void)
{
    ...
    if (UiccType == UICC_TYPE_UNKNOWN) {
        ...
        if (err == AT_NOERROR) {
            ...
            RLOGE("%s() Failed to detect card type - Retry at next request", __func__);
        } else {
            /* Legacy SIM */
             ...
             */
            UiccType = UICC_TYPE_SIM;
            RLOGI("Detected card type Legacy SIM - stored");
        }
        at_response_free(atresponse);
    }
...
static int getCardStatus(RIL_CardStatus_v6 **pp_card_status) {
    ...
    if (num_apps != 0) {
        ...
        if (uicc_type == UICC_TYPE_SIM)
            RLOGI("[Card type discovery]: Legacy SIM");
        else { /* defaulting to USIM */
            RLOGI("[Card type discovery]: USIM");
...
void setPollSIMState(void *param)
{
    ...
    if (((int) param == 1) && (enabled == 0)) {
        ...
        enabled = 1;
        RLOGD("%s() Enabled SIM status reporting", __func__);
    } else if (((int) param == 0) && (enabled == 1)) {
        at_send_command("AT*ESIMSR=0");
        enabled = 0;
        RLOGD("%s() Disabled SIM status reporting", __func__);
    }
}
...
void pollSIMState(void *param)
{
    ...
    switch (getSIMStatus()) {
    case SIM_NOT_READY:
        RLOGI("SIM_NOT_READY, poll for sim state.");
...
static int sendSimIOCmd(const RIL_SIM_IO_v6 *ioargs, ATResponse **atresponse, RIL_SIM_IO_Response *sr)
{
    ...
    if ((err < 0 || (sr->sw1 != 0x90 && sr->sw2 != 0x00)) &&
        ...
        at_response_free(*atresponse);
        *atresponse = NULL;
        RLOGD("%s() Retrying with CGLA access...", __func__);
        ...
        for (i = 0; i < count; i++) {
            if (ef_usim_files[i] == ioargsDup.fileid) {
                ...
                if (err < 0)
                    goto error;
                pathReplaced = 1;
                RLOGD("%s() Path replaced for USIM: %d", __func__, ioargsDup.fileid);
                break;
            }
        }
        if(!pathReplaced){
            ...
            for (i = 0; i < count2; i++) {
                if (ef_telecom_files[i] == ioargsDup.fileid) {
                    ...
                    if (err < 0)
                        goto error;
                    pathReplaced = 1;
                    RLOGD("%s() Path replaced for telecom: %d", __func__, ioargsDup.fileid);
                    break;
                }
            }
        }
...
void requestSetFacilityLock(void *data, size_t datalen, RIL_Token t)
{
    ...
    if (*facility_mode_str != '0' && *facility_mode_str != '1') {
        RLOGE("%s() bad facility mode!", __func__);
        goto exit;
    }
    ...
    if (err != AT_NOERROR) {
         ...
         */
        case CME_SIM_PUK_REQUIRED:
            RLOGI("PIN locked, change PIN with PUK");
            num_retries = 0;/* PUK required */
            errorril = RIL_E_PASSWORD_INCORRECT;
            break;
        /* CME ERROR 16: "Incorrect password" happens when PIN is wrong */
        case CME_INCORRECT_PASSWORD:
            RLOGI("Incorrect password, Facility: %s", facility_string);
            errorril = RIL_E_PASSWORD_INCORRECT;
            break;
         ...
         */
        case CME_SIM_PUK2_REQUIRED:
            RLOGI("PIN2 locked, change PIN2 with PUK2");
