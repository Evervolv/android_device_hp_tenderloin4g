...
int ifc_init(void)
{
    if (ifc_ctl_sock == -1) {
        ...
        if (ifc_ctl_sock < 0) {
            RLOGE("%s() socket() failed: %s", __func__, strerror(errno));
            ifc_ctl_sock = -1;
        }
    }
        ...
        in_addr_t gateway)
{
    ...
    if (ifc_up(ifname)) {
        RLOGE("%s() Failed to turn on interface %s: %s", __func__,
            ifname,
            strerror(errno));
        ifc_close();
        return -1;
    ...
    if (ifc_set_addr(ifname, address)) {
        RLOGE("%s() Failed to set ipaddr %s: %s", __func__,
    ...
    if (ifc_set_mask(ifname, netmask)) {
        RLOGE("%s() failed to set netmask %s: %s", __func__,
